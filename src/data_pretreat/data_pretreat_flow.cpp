...
DataPretreatFlow::DataPretreatFlow(ros::NodeHandle& nh) {
    ...
    // publisher
    cloud_pub_ptr_ = std::make_shared<CloudPublisher>(nh, "synced_cloud", "velo_link", 1000);
    gnss_pub_ptr_ = std::make_shared<OdometryPublisher>(nh, "synced_gnss", "map", "velo_link", 1000);
...
bool DataPretreatFlow::ValidData() {
    ...
    double diff_imu_time = current_cloud_data_.GetTime() - current_imu_data_.time;
    double diff_velocity_time = current_cloud_data_.GetTime() - current_velocity_data_.time;
    double diff_gnss_time = current_cloud_data_.GetTime() - current_gnss_data_.time;
...
bool DataPretreatFlow::PublishData() {
    ...
    cloud_pub_ptr_->Publish(current_cloud_data_.GetCloudData(), current_cloud_data_.GetTime());
    gnss_pub_ptr_->Publish(gnss_pose_, current_gnss_data_.GetTime());
