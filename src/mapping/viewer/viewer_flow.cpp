...
ViewerFlow::ViewerFlow(ros::NodeHandle& nh) {
    ...
    // viewer
    viewer_ptr_ = std::allocate_shared<Viewer>(Eigen::aligned_allocator<Viewer>());
    // viewer_ptr_ = std::shared_ptr<Viewer>(new Viewer(), Eigen::aligned_allocator<Viewer>());
...
bool ViewerFlow::ValidData() {
    ...
    double diff_odom_time = current_cloud_data_.GetTime() - current_transformed_odom_.time;
