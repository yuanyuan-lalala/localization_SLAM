...
int main(int argc, char *argv[]) {
    ...
    CloudData<PointXYZIRT>::CLOUD_PTR local_map_ptr(new CloudData<PointXYZIRT>::CLOUD());
    CloudData<PointXYZIRT>::CLOUD_PTR global_map_ptr(new CloudData<PointXYZIRT>::CLOUD());
    CloudData<PointXYZIRT>::CLOUD_PTR current_scan_ptr(new CloudData<PointXYZIRT>::CLOUD());
    ...
    while (ros::ok()) {
        ...
        if (!transform_received) {
            ...
            while (cloud_data_buff.size() > 0 && imu_data_buff.size() > 0 && gnss_data_buff.size() > 0) {
                ...
                if (!time_inited) {
                    time_inited = true;
                    init_time = cloud_data.GetTime();
                } else {
                    run_time = cloud_data.GetTime() - init_time;
                }
