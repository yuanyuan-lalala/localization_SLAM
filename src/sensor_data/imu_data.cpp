...
bool IMUData::SyncData(std::deque<IMUData,Eigen::aligned_allocator<IMUData>>& UnsyncedData, std::deque<IMUData,Eigen::aligned_allocator<IMUData>>& SyncedData, double sync_time) {
    // 传感器数据按时间序列排列，在传感器数据中为同步的时间点找到合适的时间位置
    // 即找到与同步时间相邻的左右两个数据
    // 需要注意的是，如果左右相邻数据有一个离同步时间差值比较大，则说明数据有丢失，时间离得太远不适合做差值
    // 筛除没用的数据
    while (UnsyncedData.size() >= 2) {
        //未同步的数据左边不能比同步时刻大，右边不能比同步时间小
